...
public class ProfitLossAnalysis {
    ...
    public void generateReceipt(String path, String timeStamp)
    {
        try{
          ...
          Paragraph titl = new Paragraph("TIWI URBAN TRADERS LIMITED COMPANY", FontFactory.getFont(FontFactory.HELVETICA, 20, com.itextpdf.text.Font.BOLD, BaseColor.BLACK));
          titl.setAlignment(Element.ALIGN_CENTER);
          document.add(titl);
         
          Paragraph tit2 = new Paragraph("SALES REPORT ANALYSIS FOR "+timeStamp+"", FontFactory.getFont(FontFactory.HELVETICA, 14, com.itextpdf.text.Font.BOLD, BaseColor.BLACK));
          ...
          try{
              ...
              conn = Connect.Connectdb();
                ...
                "      SUM(`item_sales`.`QuantitySold`)as QuantitySold, \n" +
                "        (`products`.`Price` * SUM(`item_sales`.`QuantitySold`)) -\n" +
                "       (`products`.`BuyingPrice` * SUM(`item_sales`.`QuantitySold`)) as Profit\n" +
